...
(function (global, factory) {
  ...
  function createPatchFunction (backend) {
    ...
    function updateChildren (parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
      // 4个游标和节点
      var oldStartIdx = 0; // 老 vnode 遍历的下标
      var newStartIdx = 0; // 新 vnode 遍历的下标
      var oldEndIdx = oldCh.length - 1; // 老 vnode 列表长度
      var oldStartVnode = oldCh[0]; // 老 vnode 列表第一个子元素
      var oldEndVnode = oldCh[oldEndIdx]; // 老 vnode 列表最后一个子元素
      var newEndIdx = newCh.length - 1; // 新 vnode 列表长度
      var newStartVnode = newCh[0]; // 新 vnode 列表第一个子元素
      var newEndVnode = newCh[newEndIdx]; // 新 vnode 列表最后一个子元素
      // 搜索相同节点时要使用
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        ...
        // 老开始和新开始对比
          ...
          // idxInOld是在老数组中的索引  // 拿到新开始的 key，在老的 children 里去找有没有某个节点有这个 key
          idxInOld = isDef(newStartVnode.key)
            ? oldKeyToIdx[newStartVnode.key]
            : findIdxInOld(newStartVnode, oldCh, oldStartIdx, oldEndIdx);
          // 不存在就是创建  // 新的 children 里有，可是没有在老的 children 里找到对应的元素
          if (isUndef(idxInOld)) { // New element
            /// 就创建新的元素
            createElm(newStartVnode, insertedVnodeQueue, parentElm, oldStartVnode.elm, false, newCh, newStartIdx);
          } else {
            // 找到了就更新 // 在老的 children 里找到了对应的元素
    ...
    // 返回的是真正的patch函数
    //oldVnode：老的虚拟 DOM 节点 vnode：新的虚拟 DOM 节点
    ...
    return function patch (oldVnode, vnode, hydrating, removeOnly) {
      // 新节点不存在，删除 // 如果新的 vnode 不存在，但是 oldVnode 存在
      if (isUndef(vnode)) {
      ...
      // 老节点不存在，创建  // 如果 oldVnode 不存在的话，新的 vnode 是肯定存在的，比如首次渲染的时候
      if (isUndef(oldVnode)) {
        ...
        if (!isRealElement && sameVnode(oldVnode, vnode)) {
          ...
          // diff发生的地方  //// 如果是 就用 patchVnode 进行后续对比 (函数后面有详解)
          ...
          // create new node
          // 将完整的vdom树转换为真实dom    // 根据新的 vnode 创建一个 DOM 节点，挂载到父节点上
          createElm(
            vnode,
            insertedVnodeQueue,
          ...
          // destroy old node  // 如果旧节点还存在，就删掉旧节点
          if (isDef(parentElm)) {
